...
// Initialize Algolia, replace with your Algolia app ID and admin API key
const client = algoliasearch("UDKPDLE9YO", "0eaa91b0f52cf49f20d168216adbad37");
const index = client.initIndex("items");

exports.getFacets = functions.https.onRequest(async (request, response) => {
  response.set("Access-Control-Allow-Origin", "*");

  try {
    const settings = await index.getSettings();
    response.send({ facets: settings.attributesForFaceting });
  } catch (error) {
    console.error("Error fetching attributesForFaceting:", error);
    response.status(500).send("Failed to fetch facets");
  }
});
