...
const Profile = () => {
  ...
  const fetchItems = async () => {
    ...
    const recentlyViewedQuery = query(collection(db, `users/${user.uid}/recentlyViewed`));
    const recentlyViewedSnapshot = await getDocs(recentlyViewedQuery);
    const recentlyViewedData = recentlyViewedSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
