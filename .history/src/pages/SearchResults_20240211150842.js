...
const SearchResults = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      const searchQuery = getSearchQuery(); // Assuming you have a function to extract this from the URL.
      if (!searchQuery) return; // Exit if no search query is provided.

      try {
        const apiUrl = `/api/ebay-search?query=${encodeURIComponent(
          searchQuery
        )}&perPage=${perPage}`;
        ...
        // Assuming eBay's response format, adjust based on actual response structure.
        setProducts(data.items || []);
      } catch (error) {
        console.error("Error fetching products: ", error.message);
        // Optionally handle the error, e.g., set an error state and display a message.
      }
    };
